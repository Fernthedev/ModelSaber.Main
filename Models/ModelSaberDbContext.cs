...
namespace ModelSaber.Database
{
    ...
    public static class DbSetExtensions
    {
        ...
        public static Task<List<Model>> GetModelAsync(this DbSet<Model> models, int? first, DateTime? createdAfter, CancellationToken cancellationToken) => Task.FromResult(models.IncludeModelData().ToList().If(createdAfter.HasValue, x => x.Where(y => y.Date > createdAfter.Value)).If(first.HasValue, x => x.Take(first.Value)).ToList());
        public static Task<List<Model>> GetModelReverseAsync(this DbSet<Model> models, int? last, DateTime? createdBefore, CancellationToken cancellationToken) => Task.FromResult(models.IncludeModelData().ToList().If(createdBefore.HasValue, x => x.Where(y => y.Date < createdBefore.Value)).If(last.HasValue, x => x.Reverse().Take(last.Value)).ToList());
