...
const webpackEnv = process.argv[process.argv.indexOf("--mode") + 1];
...
class ForkTsCheckerWarningWebpackPlugin {
    apply(compiler) {
        new forkTsChecker().apply(compiler);

        const hooks = forkTsChecker.getCompilerHooks(compiler);
...
// @remove-on-eject-begin
const getCacheIdentifier = function getCacheIdentifier(environment, packages) {
